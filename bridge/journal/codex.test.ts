...
describe("parseCodexTranscript", () => {
  ...
  test("renders current Codex image output at the output row", () => {
    const imageUrl = "data:image/png;base64,aGVsbG8=";
    const entries = parseCodexTranscript(
      [
        item({ type: "custom_tool_call", name: "exec", input: "view image", call_id: "call_image" }),
        item({
    ...
    expect(entries).toHaveLength(2);
    expect(entries[0]!.parts[0]).toMatchObject({ kind: "tool", name: "exec" });
    expect(entries[1]!.parts).toEqual([
      { kind: "image", url: imageUrl, alt: "Image read or generated by Codex" },
    ]);
  });

  test("rejects remote image URLs from the journal", () => {
    const entries = parseCodexTranscript(
      item({
        type: "message",
        role: "user",
        content: [{ type: "input_image", image_url: "https://attacker.example/image.png" }],
      }),
    );
    expect(entries).toEqual([]);
  });
  ...
  test("injected AGENTS instructions are dropped without hiding the next human turn", () => {
    const entries = parseCodexTranscript(
      [
        message("user", "# AGENTS.md instructions for /repo\n<INSTRUCTIONS>\n- internal\n</INSTRUCTIONS>"),
        message("user", "build the conversation view"),
      ].join("\n"),
    );
    expect(entries).toHaveLength(1);
    expect(entries[0]!.parts[0]).toMatchObject({ text: "build the conversation view" });
  });
