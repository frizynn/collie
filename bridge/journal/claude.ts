...
import { base64Image } from "./images.ts";
    ...
    if (typeof content === "string") {
      ...
      for (const block of content) {
        ...
        if (b.type === "text" && typeof b.text === "string") {
          ...
          const image = base64Image(b.source, "Image shared with Claude");
          ...
          if (Array.isArray(b.content)) {
            for (const resultBlock of b.content) {
              if (resultBlock === null || typeof resultBlock !== "object") continue;
              const value = resultBlock as Record<string, unknown>;
              if (value.type !== "image") continue;
              const image = base64Image(value.source, "Image read or generated by Claude");
              if (image) parts.push(image);
            }
          }
        }
