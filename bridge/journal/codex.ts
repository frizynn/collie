...
/** Keep mixed text/image content in source order. Unknown blocks remain plumbing and are skipped. */
function contentParts(content: unknown): TranscriptPart[] {
  if (typeof content === "string") {
    if (content.trim() === "") return [];
    return [{ kind: "text", ...clamp(stripAnsi(content), MAX_TEXT_CHARS) }];
  }
  if (!Array.isArray(content)) return [];
  ...
  for (const block of content) {
    ...
    if (typeof value.text === "string" && value.text.trim() !== "") {
      parts.push({ kind: "text", ...clamp(stripAnsi(value.text), MAX_TEXT_CHARS) });
      continue;
    }
    const image = inlineImage(value.image_url, "Image shared with Codex");
    if (image) parts.push(image);
  }
  return parts;
}

function outputParts(output: unknown): { text: string; images: TranscriptPart[] } {
  if (typeof output === "string") return { text: codexToolOutput(output), images: [] };
  if (!Array.isArray(output)) return { text: "", images: [] };
  const text: string[] = [];
  const images: TranscriptPart[] = [];
  for (const block of output) {
    if (block === null || typeof block !== "object") continue;
    const value = block as Record<string, unknown>;
    if (typeof value.text === "string" && value.text.trim() !== "") text.push(value.text);
    const image = inlineImage(value.image_url, "Image read or generated by Codex");
...
function isInjectedContext(text: string): boolean {
  ...
  return (
    ...
    (value.startsWith("# AGENTS.md instructions for ") && value.includes("<INSTRUCTIONS>")) ||
...
export function parseCodexTranscript(text: string): TranscriptEntry[] {
  ...
  for (const line of text.split("\n")) {
    ...
    if (p.type === "message") {
      ...
      const role = p.role;
      const parts = contentParts(p.content);
    ...
    if (p.type === "function_call" || p.type === "custom_tool_call") {
      const part: Extract<TranscriptPart, { kind: "tool" }> = {
        ...
        summary: codexToolSummary(p.arguments ?? p.input),
      };
    ...
    if (p.type === "function_call_output" || p.type === "custom_tool_call_output") {
      ...
      const parsed = outputParts(p.output);
      const outputText = stripAnsi(parsed.text);
      if (target) {
      ...
      if (parsed.images.length > 0) {
        entries.push({ uuid, ts, role: "assistant", parts: parsed.images });
      }
    }
  }
