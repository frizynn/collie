...
const UNCLAIMED: HeaderClaim = { wordmark: false, width: "full", override: false };

function sameClaim(a: HeaderClaim, b: HeaderClaim): boolean {
  return a.wordmark === b.wordmark && a.width === b.width && a.override === b.override;
}
...
export function AppHeaderHost({ bridge, error, children }: AppHeaderHostProps) {
  ...
  return (
    <HeaderSlotContext.Provider value={slots}>
      ...
      <header
        className={cn(
          "sticky top-0 z-20 flex flex-col border-b border-rule bg-background [padding-top:env(safe-area-inset-top)]",
...
export function RouteHeader({
  ...
  useLayoutEffect(() => {
    claim(owner, { wordmark, width, override: overridden });
    return () => release(owner);
  }, [claim, release, owner, wordmark, width, overridden]);
